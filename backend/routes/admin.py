...
def admin_dashboard():
    try:
        conn = get_db()
        # Fetch the last 5 reports as a preview, now including the image filename
        rows = conn.execute("""
...
def admin_register():
    try:
        ...
        # Fetch latest reports, now including the image filename
        conn = get_db()
        rows = conn.execute("""
            SELECT id, drug_name, batch_number, location, note, image_filename,
                   strftime('%Y-%m-%d %H:%M:%S', reported_on) AS reported_on, status
...
def export_drugs_word():
    # This function does not deal with reports, so no changes are needed here.
    try:
        conn = get_db()
...
def admin_reports():
    try:
        conn = get_db()
        # Updated query to fetch image_filename
        rows = conn.execute("""
            SELECT id, drug_name, batch_number, location, note, image_filename,
                   strftime('%Y-%m-%d %H:%M:%S', reported_on) AS reported_on, status
            FROM reports
...
# =========================
@admin_bp.route('/ai-trainer')
def ai_trainer_page():
    """Renders the page where admins can edit the AI's knowledge base."""
    return render_template('admin_ai_trainer.html')