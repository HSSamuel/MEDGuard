...
# =========================
# POST: Save a new counterfeit report (now with image upload)
# =========================
...
def create_report():
    ...
    conn.execute(
        """
        INSERT INTO reports (drug_name, batch_number, location, note, image_filename, reported_on, status)
        VALUES (?, ?, ?, ?, ?, ?, ?)
        """,
        (drug_name, batch_number, location, note, image_filename, datetime.now(), 0)
    )
    conn.commit()
